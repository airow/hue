...
<%def name="assistPanel(is_s3_enabled=False)">
  ...
  <script type="text/html" id="sql-context-items">
    ...
    <li><a data-bind="click: editAlias"><i class="fa fa-fw fa-info"></i> ${ _('Edit Alias') }</a></li>
   
    <li><a data-bind="click: addFields"><i class="fa fa-plus-circle"></i> ${ _('Add Fields') }</a></li>

    <li><a data-bind="click: EditMaxResultWindow"><i class="fa fa-plus-circle"></i> ${ _('Edit MaxResultWindow') }</a></li>
...
% if user.is_superuser:
    ...
    <li><a href="javascript:void(0);" data-bind="click: deleteIndex"><i class="fa fa-bolt"></i> ${ _('Delete Index') }</a></li>
% endif
%endif
<!--/ko-->
...
function addFields(){
  ...
  var url="/Index/views/addFields?indexName="+name+"&cluster="+cluster;
   /* $.post("/ES/views/addFields/", {
        indexName:ko.mapping.toJSON(name)
      }, function(msg) {
        alert(msg);
      });
*/
...
function editAlias(){
  debugger;
  ## alert("-------------------assist.mako+editAlias------------------------");
  var self = this;
  var name=self.definition.name;
  ## alert("indexname: "+name);
  var cluster=$(".assist-breadcrumb-text[title]").html();
  var s=Math.random()*10;
  var url="/Index/views/editAlias?indexName="+name+"&cluster="+cluster+"&rnd="+s;

  location.href=url;
}

function EditMaxResultWindow(){
  debugger;
  ## alert("-------------------assist.mako+EditMaxResultWindow------------------------");
  var self = this;
  var name=self.definition.name;
  ## alert("indexname: "+name);
  var cluster=$(".assist-breadcrumb-text[title]").html();
  var s=Math.random()*10;
  var url="/Index/views/EditMaxResultWindow?indexName="+name+"&cluster="+cluster+"&rnd="+s;
...
function deleteIndex(){
  ...
  var name=self.definition.name;
  var cluster=$(".assist-breadcrumb-text[title]").html();
var url="/Index/views/deleteIndex/"
var inputvar=prompt("输入 '删除'  删除"+name+". 此操作无法恢复");
...
if((strusergroup=teldUserName["responseJSON"]["WorkGroupName"])==undefined){
  ...
  strusergroup=teldUserName["responseJSON"]["WorkGroupName"];
    ...
    $.post("/Index/views/deleteIndex/", {
        indexName:ko.mapping.toJSON(name),
        cluster:ko.mapping.toJSON(cluster),
        ...
        if(objmsg["acknowledged"]==true){alert("删除成功!");
        var s=Math.random()*10;
        var newurl="http://"+location.host+"/Index/?type=elasticsearch&rnd="+s;
        ## location.reload(true);
        location.assign(newurl);
        }
        else if(objmsg["error"]["root_cause"][0]["type"]=="index_not_found_exception"){alert("索引不存在 删除失败!")}
        else{alert("删除失败!")}
      });
}
