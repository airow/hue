...
<%def name="commonHTML(indName=[],indexAlias=[], is_embeddable=False, suffix='')">
...
<script type="text/html" id="snippet${ suffix }">
  <div data-bind="visibleOnHover: { override: inFocus() || settingsVisible() || dbSelectionVisible() || $root.editorMode() || saveResultsModalVisible(), selector: '.hover-actions' }">
    ...
    <div class="airy" style="text-align: center">
          <span class="widget-label" style="display:none">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Cluster: </span>
          ## <select id="cluster" style="display:none"><option value="http://hdpjntest.chinacloudapp.cn:12200/">ESTest</option><option value="http://192.168.3.252:12200/">ESLog2</option><option value="http://192.168.2.244:12200/">ESLog</option><option value="http://192.168.2.237:12200/">ESBiz</option></select>
          <select id="cluster" style="display:none"><option value="http://hdpjntest.chinacloudapp.cn:12200/">ESTest</option><option value="http://hdpjntest.chinacloudapp.cn:12200/">ESLog2</option><option value="http://hdpjntest.chinacloudapp.cn:12200/">ESLog</option><option value="http://hdpjntest.chinacloudapp.cn:12200/">ESBiz</option></select>
        </div>
##         <script>
## alert(indexAlias)
## </script>
    
<h2 align="center" class="card-heading simple"> 修改别名 </h2>
<div class="airy" style="text-align: center">
    <table width="100%" border="0" align="right" >
          <tr >
          <td align="right" width="42%" >
          <span class="widget-label" >索引名: </span>
          </td>
          <td align="left" width="58%" >
          <input disabled="true" class="myindexname" value="${indName}" type="text" class="input-xlarge" validate="nonempty">
          &nbsp;
          <a href="#" onclick="DecInput()" 
            <i style="visibility:hidden;" class="fa fa-minus"></i>
          </a>
          </td>
          </tr>
    </table>
...
<%
arrindalias=indexAlias.split(",");
%>

  <div class="airy" style="text-align: center" id="alias">
    <table width="100%" border="0" align="right" id="tablealias">
  % for indalias in arrindalias:
          %if indalias!="":
          <tr id="tralias${indalias}">
          <td align="right" width="42%" id="td1alias${indalias}">
          <span class="widget-label" id="spanalias${indalias}">别名: </span>
          </td>
          <td align="left" width="58%" id="td2alias${indalias}">
          <input id="inputalias${indalias}" class="inputalias" value="${indalias}" type="text" validate="nonempty" >
          ## disabled="true"
          &nbsp;
          <a href="#" onclick="DecInput()" id="hrefalias${indalias}">
            <i class="fa fa-minus" id="iconalias${indalias}"></i>
          </a>
          </td>
          </tr>
          %endif
  % endfor
      ...
      <div class="airy" style="display:none" style="text-align: center">

            <span id="spanAlias1" class="widget-label" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Alias1: </span>
            <input id="Alias1" type="text" class="input-xlarge" validate="nonempty" placeholder="Alias.">
            <br />
      </div>
      ...
      <div class="airy" style="text-align: center">
          ...
          <a class="pointer" onclick="AddAliasInput()">
            <i class="fa fa-plus"></i> ${ _('Add Alias') }
          </a>     
        <br />
        </div>
...
<input align="center" type="button" class="btn" name="Create" value="更新别名" onclick="ModifyAlias()" />
</div>
<br></br>
</div>
</script>
<script> 
## var aliascount=indexAlias.length;
var aliasNum=1;
var indexJson={};
var inputIdxName="";
...
for(var i=0;i<myind.length;i++){
    removealias[i]={"remove":{"index":tempindname,"alias":myind[i]}};
}
## alert(JSON.stringify(removealias));
}

var addalias=[];

$(".inputalias").each(function(index,element){
    addalias[index]={"add":{"index":tempindname,"alias":element.value}};
})
## alert(JSON.stringify(addalias));
if((removealias&&removealias.length>0)||(addalias&&addalias.length>0)){
  myalias=removealias.concat(addalias);
}else{
  alert("不需要更新");
  return;
}
var finalalias={"actions":myalias}
## alert(JSON.stringify(finalalias));

var url0="${myurl}";
var cluster0="${mycluster}";
...
$.post("/Index/views/modifyAlias/", {
        indexName:ko.mapping.toJSON(tempindname),
        aliasJson:ko.mapping.toJSON(finalalias),
        ## oldalias:ko.mapping.toJSON(myind),
        url:ko.mapping.toJSON(url0),
        clustername:ko.mapping.toJSON(cluster0),
        ...
        ## alert(typeof(objmsg));
        if(objmsg["acknowledged"]==true){alert(tempindname+" 别名更新成功!");
        location.reload(true);
        ## var s=Math.random()*10;
        ## var newurl="http://"+location.host+"/ES/?type=elasticsearch&rnd="+s;
        ## location.assign(newurl);
        }
        else if(objmsg["error"]["root_cause"][0]["type"]=="index_not_found_exception"){alert(tempindname+" 索引不存在 别名更新失败!")}
        else{alert(tempindname+" 别名更新失败!")}
...
var inputAlias={};
if (aliasNum==1){
  var aliasName=document.getElementById("Alias1").value;
  if(aliasName!=null){
