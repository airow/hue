...
<%namespace name="common" file="workflow-common.xml.mako" />
    ...
    <action name="${ node['name'] }"${ common.credentials(node['properties']['credentials']) }${ common.retry_max(node['properties']['retry_max']) }${ common.retry_interval(node['properties']['retry_interval']) }>
        <ESIndex xmlns="uri:oozie:ESIndex-action:0.1">
            ...
            % if node['properties']['esindex_type']:
              <type>${ node['properties']['esindex_type'] }</type>
            % endif

            % if node['properties']['esindex_url']:
              <url>${ node['properties']['esindex_url'] }</url>
            % endif

            % if node['properties']['esindex_hdfs']:
              <hdfs>${ node['properties']['esindex_hdfs'] }</hdfs>
            % endif

            % if node['properties']['esindex_index']:
              <index>${ node['properties']['esindex_index'] }</index>
            % endif

            % if node['properties']['esindex_confirm']:
              <confirm>${ node['properties']['esindex_confirm'] }</confirm>
            % endif
