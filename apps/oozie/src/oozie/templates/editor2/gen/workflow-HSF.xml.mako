...
<%namespace name="common" file="workflow-common.xml.mako" />
    ...
    <action name="${ node['name'] }"${ common.credentials(node['properties']['credentials']) }${ common.retry_max(node['properties']['retry_max']) }${ common.retry_interval(node['properties']['retry_interval']) }>
        <HSF xmlns="uri:oozie:HSF-action:0.1">
            ...
            % if node['properties']['sg_url']:
              <url>${ node['properties']['sg_url'] }</url>
            % endif
            % if node['properties']['sg_logpath']:
              <logpath>${ node['properties']['sg_logpath'] }</logpath>
            % endif      
            % if node['properties']['sg_date']:
              <date>${ node['properties']['sg_date'] }</date>
            % endif
