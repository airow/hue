...
<%def name="commonHTML(templatefieldFormat=[],templatefieldType=[],templateFields=[],indexAlias=[],tempMaxResultWindow=[],tempNumofShards=[],tempNumofRep=[],indName=[],orgcluster=[],is_embeddable=False, suffix='')">
...
<br></br>
## font-weight:bold;
<h2 align="center" class="card-heading simple"> 修改模板 </h2> 
...
<table class="table datatables" align="center" width="900" border="0" cellpadding="0" cellspacing="0" id="tabProduct0">    
    ...
    <tr onclick="SetRowCanEdit(this)">    
      ...
      <td bgcolor="#FFFFFF" EditType="DropDownList" id="cluster" DataItems="{text:'eslog',value:'eslog'},{text:'esbiz',value:'esbiz'}"></td>
      <td bgcolor="#FFFFFF" id="idxname">${indName}</td>        
      <td  bgcolor="#FFFFFF" EditType="TextBox" id="shardsnum"></td>       
    </tr>  
    <tr> 
    ...
    <tr onclick="SetRowCanEdit(this)">    
      ...
      <td bgcolor="#FFFFFF" EditType="TextBox" id="replicasnum"></td>
      ...
      ## </td>    
      <td bgcolor="#FFFFFF" EditType="TextBox" id="maxresultwindow"></td>
      ...
      ## </td>     
      <td bgcolor="#FFFFFF" EditType="TextBox" id="Alias"></td>   
    </tr>    
</table>   
...
<%
strtempFields=templateFields.split(",");
strtempfieldType=templatefieldType.split(",")
strtempfieldFormat=templatefieldFormat.split(",")
i=0
%>   
    <tr>    
      <th width="32" align="center" bgcolor="#EFEFEF" Name="Checked"><input style="display:none" type="checkbox" name="checkbox" value="checkbox" /></th>    
      <th width="186" bgcolor="#EFEFEF" Name="FieldName" EditType="TextBox">字段名</th>    
      <th width="186" bgcolor="#EFEFEF" Name="DataType" EditType="DropDownList" DataItems="{text:'text',value:'text'},{text:'keyword',value:'keyword'},{text:'AboveTwo',value:'AboveTwo'},{text:'integer',value:'integer'},{text:'long',value:'long'},{text:'short',value:'short'},{text:'double',value:'double'},{text:'float',value:'float'},{text:'boolean',value:'boolean'},{text:'date',value:'date'},{text:'range',value:'range'}">数据类型</th>    
      ## <td width="153" bgcolor="#EFEFEF" Name="OtherType" EditType="TextBox">其他类型</td>
      <th width="155" bgcolor="#EFEFEF" Name="Format" EditType="TextBox">格式</th>    
    ...
    % for temfield in strtempFields:
<%      
datatypeid="datatype"+str(i)
formatid="format"+str(i);
print("-----------formatid:"+formatid)
%>
    <tr onclick="SetRowCanEdit(this)">    
      <td align="center" bgcolor="#FFFFFF"><input type="checkbox" name="checkbox2" value="checkbox" /></td>    
      <td bgcolor="#FFFFFF" Name="FieldName">${temfield}</td>
      ## DataItems="{text:'text:模糊匹配，全文搜索',value:'text'},{text:'keyword:全词匹配，可排序，可聚合',value:'keyword'},{text:'以上两项',value:'two'},{text:'Integer',value:'integer'},{text:'Long',value:'long'},{text:'Short',value:'short'},{text:'Double',value:'double'},{text:'Float',value:'float'},{text:'Boolean',value:'boolean'},{text:'Date',value:'date'},{text:'Range',value:'range'}    
      <td bgcolor="#FFFFFF" Name="DataType" class="datatype" id=${datatypeid} Value="${strtempfieldType[i]}" EditType="DropDownList">${strtempfieldType[i]}</td>    
      ## <td bgcolor="#FFFFFF"></td>    
      <td bgcolor="#FFFFFF" id=${formatid} Name="Format">${strtempfieldFormat[i]}</td>    
...
<br />    
<input align="center" type="button" class="btn" name="Submit" value="新增" onclick="AddRow(document.getElementById('tabProduct'),1)" />    
<input align="center" type="button" class="btn" name="Submit2" value="删除" onclick="DeleteRow(document.getElementById('tabProduct'),1)" />    
## <input align="center" type="button" class="btn" name="Submit22" value="重置" onclick="window.location.reload()" />    
...
<div style="text-align:center">
<input align="center" type="button" class="btn" name="Create" value="更新" onclick="uploadTemplateJson()" />
...
<script type="text/javascript">
  var clustername="${orgcluster}";
  ## var myselect = document.getElementById("cluster");
  window.onload=function(){
  if(clustername=="eslog"){
    document.getElementById("cluster").innerText = "eslog"
  }
  else{
    document.getElementById("cluster").innerText = "esbiz"             
  }
  tempMaxResultWindow="${tempMaxResultWindow}"
  tempNumofShards="${tempNumofShards}"
  tempNumofRep="${tempNumofRep}"
  tempAlias="${indexAlias}"
  if(tempMaxResultWindow!=null&&tempMaxResultWindow!=""&&tempMaxResultWindow!="None"){
    document.getElementById("maxresultwindow").innerText=tempMaxResultWindow
  }
    if(tempNumofShards!=null&&tempNumofShards!=""&&tempNumofShards!="None"){
    document.getElementById("shardsnum").innerText=tempNumofShards
  }
    if(tempNumofRep!=null&&tempNumofRep!=""&&tempNumofRep!="None"){
    document.getElementById("replicasnum").innerText=tempNumofRep
  }
    if(tempAlias!=null&&tempAlias!=""&&tempAlias!="None"){
      document.getElementById("Alias").innerText=tempAlias
    }
...
function DecInput() {  
  ...
  var table = document.getElementById("tablealias");
  
  var hrefid=document.activeElement.id;
  var aliasname=hrefid.substring(9,);
  var iconid="iconalias"+aliasname;
  var whichicon=document.getElementById(iconid);
  
  var spanid="spanalias"+aliasname;
  var whichspan=document.getElementById(spanid);
  var inputid="inputalias"+aliasname;
  var whichinput=document.getElementById(inputid);
  var trid="tralias"+aliasname;
  var whichtr=document.getElementById(trid);
  var td1id="td1alias"+aliasname;
  var whichtd1=document.getElementById(td1id);
  var td2id="td2alias"+aliasname;
  var whichtd2=document.getElementById(td2id);

  whichtd1.remove(whichspan);
  whichtd2.remove(whichinput);
  whichtd2.remove(whichicon);

  whichtr.remove(whichtd1);
  whichtr.remove(whichtd2);
  ## whichtr.remove(whichicon);
  ## table.remove(whichtr);
...
function AddAliasInput(table, index){    
  ...
  tr1.id="tralias"+aliasNum;
  ## question.appendChild(tr1);
  table1.appendChild(tr1);

  var td1=document.createElement("td");
  td1.id="td1alias"+aliasNum;
  td1.width="42%";
  td1.align="right";
  tr1.appendChild(td1);
  ...
  input.type = "text";  
  input.width="80";
  input.id = "inputalias" + aliasNum;  
  input.name = "Alias" + aliasNum;
	input.className="inputalias";
  ...
  $(td2)[0].innerHTML += "&nbsp;";
  ## $(td2).html($(td2).html()+"&nbsp;")"&amp;nbsp;";
  var $dlgStr = $("<a href=\"#\" onclick=\"DecInput()\" id=\"hrefalias"+aliasNum+"\">"
            +"<i class=\"fa fa-minus\" id=\"iconalias"+aliasNum+"\"></i></a>");
	var $parent=$(td2);
  $parent.append($dlgStr);
			## $('.dialog-img').media({
			## 	width : 700,
			## 	height : 700
			## });
...
var num_count=parseInt(lastRow.cells[2].id.substr(8))+1
## var str_count=num_count.toString();
newRow.cells[2].id="datatype"+num_count;
newRow.cells[2].class="datatype";
## alert(num_count)
newRow.cells[3].id="format"+num_count;
...
flag=3;
indexJson={}
inputCluster=document.getElementById("cluster").innerText;
...
var ind_template=inputIdxName.indexOf("_template");
if(ind_template!=-1){
  inputIdxName=inputIdxName.substring(0,ind_template);
}
indexName=inputIdxName;
...
var inputAlias={};
## $(".inputalias").each(function(index,element){
##   if(element.value!=""){
##     inputAlias[element.value]={};
##   }
    
## })

var aliasName=document.getElementById("Alias").innerText.toString();
...
for(var j=0;j<len;j++){
  ...
  if(JSON.stringify(tableData[j].DataType)==JSON.stringify("AboveTwo")){
    var contenttypeDef={};
...
var tempindexName={"properties":properties};
var mappings={};
mappings["_default_"]=tempindexName;
...
## var partitionGranularity=document.getElementById("partitionGranularity").value;
...
var url="/Template/views/getJson/";
    ...
    $.post("/Template/views/getJson/", {
        ...
        if(objmsg["acknowledged"]==true){alert("更新成功!");
        ## var s=Math.random()*10;
        ## var newurl="http://"+location.host+"/Template/?type=template&rnd="+s;
        ## location.assign(newurl);
        location.reload(true);
        }
        else if(objmsg["error"]["root_cause"][0]["type"]=="index_already_exists_exception"){alert("更新失败!");}
        else{alert("更新失败!");}
      });
