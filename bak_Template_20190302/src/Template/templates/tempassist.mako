...
## <span data-bind="text: 'views/addFields/'+definition.name"></span>
    <!-- ko if: typeof definition !== 'undefined' -->
    
    <li><a data-bind="click: editTemplate"><i class="fa fa-fw fa-info"></i> ${ _('Edit Template') }</a></li>
    <!--ko if: sourceType !=='template' -->
...
function addFields(){
  ...
  var url="/ES/views/addFields?indexName="+name+"&cluster="+cluster;
...
function editTemplate(){
  debugger;
  ## alert("-------------------assist.mako+editTemplate------------------------");
  var self = this;
  var name=self.definition.name;
  ## var ind=name.indexOf("_template")
  ## alert(ind)
  ...
  var url="/Template/views/editTemplate?templateName="+name+"&cluster="+cluster;
...
function deleteIndex(){
  ...
  var name=self.definition.name;
  var cluster=$(".assist-breadcrumb-text[title]").html();
var url="/ES/views/deleteIndex/"
var inputvar=prompt("输入 '删除'  删除"+name+". 此操作无法恢复");
if (inputvar=="删除"){
    $.post("/ES/views/deleteIndex/", {
        indexName:ko.mapping.toJSON(name),
