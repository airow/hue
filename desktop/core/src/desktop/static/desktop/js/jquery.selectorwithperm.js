...
(function ($, window, document, undefined) {
  ...
  var pluginName = "jHueSelectorWithPerm",
      defaults = {
  ...
  function Plugin(element, options) {
    ...
    this._perm = window.permssionsMapping
    this.init();
  }
  ...
  Plugin.prototype.init = function () {
    ...
    $(_this.element).find("option").each(function (cnt, opt) {
      var text = $(opt).text();
      var app = text.split(".")[0];
      var initial = $(opt).text().substr(0, 1).toLowerCase();
      initial = app.toLowerCase();
    ...
    if (sortedKeys.length == 0) {
      ...
      for (var i = 0; i < sortedKeys.length; i++) {
        ...
        var permConf = _this._perm[key] || { name: key.toUpperCase() };
        permConf.action = permConf.action || {};
        permConf.action.access = permConf.action.access || "启用";
        dividerLi.html("<strong>" + permConf.name + "</strong>");
